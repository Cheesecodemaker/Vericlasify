...
// Import files module constants (lazy load to avoid circular deps)
let filesModule = null;
function getFilesModule() {
    if (!filesModule) {
        filesModule = require('./files');
    }
    return filesModule;
}
 ...
 * @param {string[]} excludePatterns - Patterns to exclude (e.g., ['.vericl'])
 * @returns {string[]} - Array of file paths
 */
function getAllFiles(dirPath, excludePatterns = []) {
    const files = [];
    // Exclude the .vericl folder, legacy files, and common folders
    const defaultExcludes = ['.vericl', '.pinesu.json', '.regpinesu.json', '.git', 'node_modules', '.pinesuhistory'];
...
function getEncryptionConfig(dirPath) {
    // New location: .vericl/encryption.json
    const newConfigPath = path.join(dirPath, '.vericl', 'encryption.json');
    // Legacy location: .vericl-encryption.json
    const legacyConfigPath = path.join(dirPath, '.vericl-encryption.json');

    // Try new location first
    if (fs.existsSync(newConfigPath)) {
        try {
            return JSON.parse(fs.readFileSync(newConfigPath, 'utf-8'));
        } catch {
            return null;
        }
    }
...
function saveEncryptionConfig(dirPath, config) {
    // Ensure .vericl folder exists
    const vericlFolder = path.join(dirPath, '.vericl');
    if (!fs.existsSync(vericlFolder)) {
        fs.mkdirSync(vericlFolder, { recursive: true });
    }
    const configPath = path.join(vericlFolder, 'encryption.json');
