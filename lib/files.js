...
module.exports = {
    ...
    createRegistration: (el) => {
        ...
        const regPinesuPath = path.join(process.cwd(), '.regpinesu.json');
        if (fs.existsSync(regPinesuPath)) {
            ...
            const historyFileName = 'regpinesu' + contentDate.toISOString().replace(/:/g, '-') + '_' + rootIdentifier + '.json';
            fs.writeFileSync(path.join(process.cwd(), '.pinesuhistory', historyFileName), contentJSON);
        }

        try {
            fs.writeFileSync(filePath + "/.regpinesu.json", JSON.stringify(el));
        } catch (err) {
    ...
    createPineSUOld: () => {
        const historyDir = path.join(process.cwd(), '.pinesuhistory');
