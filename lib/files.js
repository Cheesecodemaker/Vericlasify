...
module.exports = {
    ...
    createRegistration: (el) => {
        ...
        if (fs.existsSync(cwd + "/" + regFile)) {
            ...
            let contentDate;
            try {
                contentDate = content.date ? new Date(content.date) : new Date();
                if (isNaN(contentDate.getTime())) {
                    contentDate = new Date(); // Fallback to current date
                }
            } catch (e) {
                contentDate = new Date(); // Fallback to current date
            }
            const dateStr = contentDate.toISOString().replace(/[:.]/g, '-');
            fs.writeFileSync(cwd + "/" + historyDir + "/regvericl_" + dateStr + "_" + (content.root || 'unknown') + ".json", contentJSON);
        }
